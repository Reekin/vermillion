...
主导航上方是 **工作台** 和 **Inbox**，底部是同级的 **设置**。工作台是主页，用于讨论、修改文档、开工和查看执行；分页布局、会话列表筛选和 workspace 范围见 [工作台](../Workbench/Think/PRD.md)。Inbox 集中展示待决策和已合入内容。设置以弹窗打开，提供新会话引擎、标题模型与引擎程序路径，见[会话引擎](../Foundation/Engines/PRD.md)。
...
左侧列出用户发起的会话树和 agent 独立启动的会话，从讨论 fork 出的 Worker 分支在所属会话树里查看，见[会话列表](../Workbench/Think/PRD.md#会话列表)与[会话树](../Workbench/Think/PRD.md#会话树)。所有会话链接都进入工作台 → 会话，并选中目标会话或树内节点。执行分支的输入框上方显示[当前工作状态条](../Workbench/Think/PRD.md#当前工作状态条)。
