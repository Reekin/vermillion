...
- **Worker**执行一张工单，按允许范围修改。开发工单使用独立分支和 worktree，独立工单在 workspace 根执行。验收方法按改动性质自选；涉及界面时通过工作台起一个独立桌面上的实例，用户屏幕上不会出现。Worker 自行邀请 reviewer，判断哪些意见适用，最多两轮；随后由空白 subagent 按验收条目和文档原文验证真实结果，实例的 CDP 地址由 Worker 提供。只许少做，不许多做；不读其他工单的 worktree。
- **Supervisor**每个 Worker turn 结束后检查工单、输出和 diff，发现偏离范围、反复无效重试或提前交付时提醒或中断。它只能把执行拉回目标，不能追加工作。
