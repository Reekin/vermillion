...
监工角色头部的 `checkIntervalMinutes` 指定每轮检查结束到下一轮开始的间隔，单位为分钟，取正整数；未配置时为 5 分钟。该参数只影响监工检查，不改变模型配置或 Worker 并发。

使用全局角色时读取全局值；项目 `append` 未填写时继承全局，填写后只覆盖间隔；项目 `override` 使用项目值，未填写则使用默认值。角色编辑窗口的监工配置提供“检查间隔（分钟）”，全局模式只读，项目模式可编辑，清空表示按当前模式继承或使用默认值；保存与重新打开保持一致，正文不显示头部字段。

通过角色编辑器或角色 CLI 保存间隔后，空闲监工的下次检查按上一轮结束时间和新间隔重新计算；已到期时只检查一次。运行中的检查不被打断，结束后使用新间隔；暂停与自动推进开关仍按工作规则生效。直接编辑角色文件的值在工作台下一次读取配置时生效。
