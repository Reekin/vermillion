...
- 工作台左侧会话列表常驻，新建会话旁放「全部」/ workspace 筛选下拉；筛选只影响列表，不切换当前会话。具体 workspace 筛选作为新草稿的默认项目。
- 右侧顶部分页固定为会话、工作、文档、领域、角色、Issues、自动化、管理。会话默认打开，SessionPane 与右侧 Docs Explorer 在切页时保持挂载并保留草稿。
- 其他分页在标题栏显示 workspace 选择器，明确编辑范围；「全部」仅是会话列表筛选值。页面内容不再嵌套导航分页；对象列表（领域）可用左列主从布局，左列固定宽度，右侧详情限制在可读宽度内，区块标题与其操作按钮左对齐相邻，不把按钮推到页面最右。领域草稿在切页时保留。
- 工作和工单链接进入工作台 → 工作并定位目标；会话链接进入工作台 → 会话并选中目标会话或树内节点。Inbox 保持弹窗与展开页面行为，设置只以弹窗打开。
...
- 界面自有文案（导航、分页、区块标题、按钮、菜单项、空态和状态说明）用中文，Docs、Domain、Automation 分别写作文档、领域、自动化。对象标识、角色身份和技术产物保留原文：workspace、Worker / Maintainer / Liaison、commit 与 diff、id、路径、模型名、引擎名；Inbox 与 Issue / Issues 也用原名。
...
- 改公共组件或 `app.css` 后，起实例打开 工作台 → 会话（会话列表 + 文档）、Inbox、工作台 → 工作 三个页面，看普通状态、空态、长中文标题、错误和窄窗口。截图交给空白 subagent 对照本文判断，不自己看图下结论。
