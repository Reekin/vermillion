...
- 主导航上方是工作台和 Inbox，设置位于底部且与两者同级。设置以弹窗打开，包含新会话引擎、标题模型与各引擎程序路径，见[会话引擎](../Engines/PRD.md)。
- 工作台左侧会话列表常驻，New Chat 旁放 All / workspace 筛选下拉；筛选只影响列表，不切换当前会话。具体 workspace 筛选作为新草稿的默认项目。
- 右侧顶部分页固定为会话、工单、Docs、Domain、角色、Issues、Automation、管理。会话默认打开，SessionPane 与右侧 Docs Explorer 在切页时保持挂载并保留草稿。
- 其他分页在标题栏显示 workspace 选择器，明确编辑范围；All 仅是会话列表筛选值。页面内容不再嵌套导航分页；对象列表（Domain）可用左列主从布局，左列固定宽度，右侧详情限制在可读宽度内，区块标题与其操作按钮左对齐相邻，不把按钮推到页面最右。Domain 草稿在切页时保留。
- 工单链接进入工作台 → 工单并定位目标；会话链接进入工作台 → 会话并选中目标会话或树内节点。Inbox 保持弹窗与展开页面行为，设置只以弹窗打开。

## 视觉规则

- 单色、低饱和；主操作靠明度和位置突出，错误用 `InlineNotice tone="error"` 或文字说明，不用红色。会话树节点与会话列表行使用同一组状态色：低饱和黄色表示进行中、绿色表示已完成未读，已读为中性色；Worker 节点显示 W，当前位置轮廓独立于状态色。
...
- 破坏性动作（取消工单、删除领域）不直接摆在行或卡片上：行级的收成 hover 才显示的 × 图标且固定占位，卡片级的收进「···」菜单。删除领域、移除 workspace 这类不可撤销动作点击后弹窗确认。查看类动作（会话、来源）用带 hairline 描边的 `Button variant="ghost"`，和旁边的元信息文字分得开。
- 列表行的尾部动作列宽度固定，行与行之间的按钮、状态列必须对齐；hover 只改透明度和底色，不改布局。
- 时间在快速查看场景用相对时间（3 分钟前、昨天 22:10），页面模式可附完整时间和 commit。
- 弹窗承载的面板不放左侧导航列；需要切换对象时用标题栏里的下拉。开关、下拉这类单值配置直接放在页面上改动即保存；只有多选树、多行条目才进弹窗。目录勾选树使用三态复选框：子级部分勾选时父级显示半选，勾选父级即覆盖全部子级。弹窗展开为页面时保持当时所在的分页、选中项和展开状态。
