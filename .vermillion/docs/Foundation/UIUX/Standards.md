...
- 工单状态用 `Badge` 的状态形态表达，五种一眼可分：待验收 浅底胶囊 + 实心点（最显眼，因为需要用户动手）、待决策 描边胶囊 + 实心点、进行中 无框 + 粗空心环、排队中 无框 + 细空心环、已关闭 / 已取消 灰字无点。所有形态尺寸相同，无框的也保留透明边框，保证同列对齐。
- 破坏性动作（取消工单、取消任务、移除 workspace）不直接摆在行或卡片上：行级的收成 hover 才显示的 × 图标且固定占位，卡片级的收进「···」菜单。查看类动作（会话、管家会话）用带 hairline 描边的 `Button variant="ghost"`，和旁边的元信息文字分得开。
