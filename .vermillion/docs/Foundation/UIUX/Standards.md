...
- 工单状态文案为准备中、排队中、进行中、等待合入、等待用户、已关闭、已取消，由 `task-labels.ts` 集中提供。`Badge` 的状态形态统一在 `app.css` 实现：等待用户为描边胶囊加实心点，进行中为无框加粗空心环，排队中为无框加细空心环，其余状态用无框文字。所有形态尺寸相同，无框的也保留透明边框，保证同列对齐。
