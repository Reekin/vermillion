...
会话内消息、工具调用和终端流的实体标识只有一种：由会话 id 与引擎 item id 组合得到，实时事件与历史读取对同一个 item 生成同一个 id。同一 item 无论先经流式事件进入投影、还是随后由 hydration 再次写入，都落到同一实体上；投影合并只按 id 对齐，不做按文本匹配的替换或去重。
