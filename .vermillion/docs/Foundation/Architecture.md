...
产品行为见[工作台 · 搜索](../Workbench/Think/PRD.md#搜索)。rollout 扫描由随包附带的 ripgrep 子进程执行，扫描范围是会话索引解析出的本 workspace rollout 文件；是否属于 Vermillion 会话在出现命中的文件上判定，不预先通读全部文件。工单与文档的体量小，仍在工作台服务内直接匹配。

命中通过已有的 `WorkbenchEvent` 通道按批增量推送给界面。一次查询的生命周期由服务持有：查询被取代或发起方不再需要时终止它的扫描进程，过期查询的命中不再推送。`search.query` 保留为等待扫描结束、一次返回完整结果的调用，供 CLI 使用。

命中上下文在服务侧截成有界窗口后再推送，超长行不整行进入结果。
