...
const boot = async (): Promise<void> => {
  ...
  const localEndpoint = await startLocalEndpoint(persistenceBaseDir, async (request) => {
    ...
    if (["engine.listModels", "sessionBrowser.open", "sessionBrowser.rename", "chatTree.get", "chatTree.nodeAction", "chatTree.submit", "chatTree.retry", "chatTree.cancel", "chatTree.remove", "chatTree.operations", "chatTree.markRead"].includes(request.method)) {
