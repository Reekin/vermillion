...
export const IntegrationControls = ({ client, workspaceId, workItemId, action, item, showStatus = true }: {
  ...
  return <div className="mt-3 space-y-2 border-t border-border pt-3">
    ...
    {action.agent && item?.run.retryAt && <p className="text-caption text-muted-foreground">执行已失败 {item.run.attempts} 次；下次：{new Date(item.run.retryAt).toLocaleString("zh-CN")}</p>}
