...
1. 左侧 rail 点 **Workspaces**，用 + 添加一个项目目录。Vermillion 会在目录里初始化 git（若尚无）并写入 `docs/AGENTS.md`（设计伙伴的工作规则）。
2. 回到 **思考**，左上角选 workspace，在 SESSIONS 里按 + 新建会话，和设计伙伴聊需求。它只会改 `docs/` 下的文件。
3. 右栏 **待确认变更** 会列出 `docs/` 的改动；点文件名可直接编辑并保存。
...
- 每个 workspace：`<root>/.Vermillion/`（missions / workitems / decisions / issues，一条一个 JSON），`<root>/docs/`（真相源文档，走 git）
