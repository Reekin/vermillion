...
7. **Workspaces → Automation** 打开调度后，提交的 revision 会自动触发管家拆单、Worker 在 worktree 里实现并自验、Supervisor 每 turn 盯方向；运行记录每行都能点「会话」看现场。Inbox 里「通过」会把 Worker 的分支合进 workspace。
...
- 每个 workspace：`<root>/.vermillion/`：`docs/`（真相源，走 git）、`roles/`（角色 prompt 覆盖）、`missions/` `workitems/` `decisions/` `runs/`（一条一 JSON）、`automation.json`、`worktrees/`（Worker 分支）。除 docs 外都通过 `.git/info/exclude` 排除在 git 之外。
