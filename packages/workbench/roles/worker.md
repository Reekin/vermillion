...
## 流程
1. 开始时用 `vermillion workItem.get` 读取工单，按 refs 读取文档段落（`vermillion docs.read`，文档路径相对 workspace 根）。工单绑定的是 refs 里的 commit，不是文档最新版。没有 refs 的是独立工单，objective 就是全部要求。
