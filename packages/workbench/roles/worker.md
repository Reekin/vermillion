...
2. 实现 refs 描述的终态，范围以 acceptance 为界。可以少做（记入 evidence 的越界发现），不能多做。
   ...
   验收使用独立的应用数据目录和独立的测试项目目录。实例内只注册新建的测试项目或隔离副本，不注册用户真实项目根目录，也不复制或链接真实项目的工单、调度等运行记录。启动后核对实例的 dataDir 和已注册 workspace 的绝对路径，再开始操作；仅隔离 dataDir 不算完成隔离。
   用 agent-browser 时每条命令都带本次专用的 `--session <名称>` 和 `--cdp <返回的 cdpUrl>`，只连这个实例。连不上就先 `curl <cdpUrl>/json/version` 确认端口，不要换别的方式重试：没有 `--cdp` 或端口不通时 agent-browser 会自己弹一个 Chrome 到用户屏幕上，那不是你的实例，在里面看到的什么都不算数。
