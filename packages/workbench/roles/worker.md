...
2. 实现 refs 描述的终态，范围以 acceptance 为界。可以少做（记入 evidence 的越界发现），不能多做。
   验收方法按改动性质自己定：改了界面才起实例看一眼（隔离的数据目录，用完关掉），改了 CLI 跑命令，纯逻辑跑相关测试。不跑全量测试套件，不写 evidence 文件夹，截图只在必须时留一两张。
...
4. 拉起一个空白 verifier subagent 做封闭式验收（prompt 同样用 `role.read` 获取 verifier）：给它 acceptance 列表、refs 指向的文档原文（`docs.read` 带 commit）、diff，以及跑起来的应用（仅当改动涉及界面）。任一条 fail 就修复后重跑 verifier，不修改 acceptance。
...
决策卡是给用户看的，不是调试报告：`question` 一句人话说卡在哪；`context` 两三句说发生了什么、为什么需要用户定；每个选项的 `label` 是动作，`detail` 写选了会怎样；`recommended` 写推荐哪个和一句理由。源码路径、日志、验收报告不要放进去。
