...
export const workbenchRpc = {
  ...
  "app.start": {
    params: z.object({ targetPath: z.string().min(1), expectedRevision: z.string().min(1).optional(), expectedBuildId: z.string().min(1).optional(), dataDir: z.string().min(1).optional(), port: z.number().int().min(1024).max(65535), fixture: z.enum(["session-tree", "real-session"]).optional(), codexConfigSource: z.string().min(1).optional(), env: z.record(z.string()).optional() }),
  ...
  "app.stop": { params: z.object({ dataDir: z.string().min(1), pid: z.number().int().positive(), instanceId: z.string().min(1), keepData: z.boolean().optional() }), result: z.object({ dataDir: z.string(), pid: z.number().int().positive(), stopped: z.literal(true), portReleased: z.literal(true), warnings: z.array(z.string()).optional() }) },
  "app.window": {
