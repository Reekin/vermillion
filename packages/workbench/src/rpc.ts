...
export const workbenchRpc = {
  ...
  "app.stop": { params: z.object({ dataDir: z.string().min(1), pid: z.number().int().positive(), instanceId: z.string().min(1) }), result: z.object({ dataDir: z.string(), pid: z.number().int().positive(), stopped: z.literal(true), portReleased: z.literal(true), warnings: z.array(z.string()).optional() }) },
  "app.window": {
