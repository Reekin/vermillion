...
export const createWorkbenchRpcHandler = (service: WorkbenchService) => {
  const handlers: Handlers = {
    ...
    "session.messages.pending": (p) => service.listPendingSessionMessages(p.sessionId),
    "session.messages.cancel": (p) => service.cancelPendingSessionMessage(p.sessionId, p.messageId),
    ...
    "work.confirm": (p) => service.confirmWorkRequestDelivery(p.workspaceId, p.requestId),
    "work.pause": (p) => service.pauseWork(p.workspaceId, p.requestId),
    "work.resume": (p) => service.resumeWork(p.workspaceId, p.requestId),
    "work.continueFrom": (p) => service.continueWorkFrom(p.workspaceId, p.requestId, p),
    ...
    "workItem.retry": (p) => service.retryWorkItem(p.workspaceId, p.workItemId),
    "workItem.continueFrom": (p) => service.continueWorkItemFrom(p.workspaceId, p.workItemId, p),
    "workItem.confirm": (p) => service.confirmWorkItemDelivery(p.workspaceId, p.workItemId),
