...
import { effectiveNeeds, actionIsOpen, type WorkflowAction } from "./contracts.js";
...
export class WorkbenchService {
  ...
  async finishPreparation(workspaceId: string, sessionId: string, turnId?: string): Promise<void> {
    ...
    for (const item of await this.listWorkItems(workspaceId)) {
      if (item.status === "preparing" && item.requestId === request.requestId) {
        ...
        await this.mutateWorkItem(workspaceId, item.workItemId, (current) => ({ ...current, status: "queued",
          run: { ...current.run, ...(!current.run.sessionId ? { forkSessionId: sessionId, forkTurnId } : {}) } }));
      }
    }
  ...
  async refreshActions(workspaceId: string): Promise<void> {
    ...
    for (const item of items) {
      const actions = (await this.listActions(workspaceId)).filter((a) => a.workItemIds.includes(item.workItemId) && actionIsOpen(a));
      ...
      if (item.status === "merging" && !actions.some((a) => a.kind === "integration"))
        await this.createAction(workspaceId, { kind: "integration", role: "workbench", ownerKey: "integration", workItemIds: [item.workItemId], status: "pending", stage: "merge", message: "继续合入已验收成果。", integration: { operation: "merge", diffStat: "" } });
      ...
      if (!actions.some((a) => a.kind === "execute") && !(await this.isWorkItemBlocked(workspaceId, item.workItemId)))
        await this.createAction(workspaceId, { kind: "execute", role: "worker", ownerKey: "worker:" + item.workItemId,
          workItemIds: [item.workItemId], sessionId: item.run.sessionId, status: "pending", stage: "open",
          message: item.run.resumeMessage ?? "读取工单，在允许范围内执行并提交验收结果。" });
    }
  }
  ...
  async isWorkItemBlocked(workspaceId: string, workItemId: string): Promise<boolean> {
    ...
    return item.dependsOn.some((id) => items.find((other) => other.workItemId === id)?.status !== "closed")
      || (await this.listActions(workspaceId)).some((action) => actionIsOpen(action) && action.workItemIds.includes(workItemId) && action.kind === "integration")
  ...
  async failAction(workspaceId: string, actionId: string, failure: string): Promise<WorkflowAction> {
    ...
    if (delay === undefined && !(await this.listDecisions(workspaceId)).some((card) => card.actionId === actionId && !card.answer && !card.withdrawn)) {
      await this.createDecision(workspaceId, { actionId, kind: "attempts", workItemId: action.workItemIds[0], sessionId: action.sessionId,
  ...
  async createWorkItem(
    ...
    const item = await store.workItems.put({
      ...
      rejections: [],
      decisions: [],
      run: { sessionId: input.sessionId, worktreePath: input.worktreePath, branch: input.branch },
      createdAt: now,
      updatedAt: now
    });
  ...
  async startWorkItem(workspaceId: string, workItemId: string, run: WorkItem["run"]): Promise<WorkItem> {
    ...
    return this.mutateWorkItem(workspaceId, workItemId, (item) => ({
      ...item,
      status: "running",
      run: { ...item.run, ...run, baseCommit, retryAt: undefined, staleTurnId: undefined }
    }));
  }
  ...
  private async submitResult(workspaceId: string, workItemId: string, input: Parameters<WorkbenchService["submitWorkItem"]>[2]): Promise<WorkItem> {
    ...
    await this.createAction(workspaceId, { kind: "integration", role: "workbench", ownerKey: "integration", workItemIds: [workItemId], status: "pending", stage: "merge", message: "验收通过，等待合入。", integration: { operation: "merge", diffStat: "" } });
  ...
  private async drainIntegrations(workspaceId: string): Promise<void> {
    ...
    const { docs } = await this.context(workspaceId);
    const pending = actions.filter((a) => a.kind === "integration" && actionIsOpen(a) && a.status !== "decision" && (!a.retryAt || a.retryAt <= this.now()));
      ...
      try {
        ...
        if (action.stage === "cleanup") {
          ...
          await this.mutateWorkItem(workspaceId, workItemId, (current) => ({ ...current, status: integration.operation === "cancel" ? "cancelled" : integration.operation === "rollback" ? "queued" : "closed",
            ...
            run: { ...current.run, worktreePath: undefined, branch: undefined, baseCommit: integration.operation === "rollback" ? integration.commit : current.run.baseCommit,
              resumeMessage: integration.operation === "rollback" ? "用户回滚：" + integration.reason + "。重新判断隔离目录，需要时自行创建并通过 workItem.update 登记。" : current.run.resumeMessage } }));
  ...
  private async returnWorkItem(workspaceId: string, workItemId: string, reason: string): Promise<WorkItem> {
    return this.mutateWorkItem(workspaceId, workItemId, (item) => {
      return { ...item, status: "queued", evidence: undefined, verify: undefined, rejections: [...item.rejections, { reason, at: this.now() }], run: { ...item.run, staleTurnId: undefined, resumeMessage: [item.run.resumeMessage, reason].filter(Boolean).join("\n") } };
    });
  }
  ...
  private async rollbackResult(workspaceId: string, workItemId: string, reason: string): Promise<WorkItem> {
    ...
    if (!(await this.listActions(workspaceId)).some((a) => a.kind === "integration" && a.workItemIds.includes(workItemId) && actionIsOpen(a))) {
      await this.createAction(workspaceId, { kind: "integration", role: "workbench", ownerKey: "integration", workItemIds: [workItemId], status: "pending", stage: "rollback", message: "用户回滚：" + reason.trim(), integration: { operation: "rollback", target: item.merge.commit, targets: item.merge.commits, diffStat: "", reason: reason.trim() } });
    }
    await this.drainIntegrations(workspaceId);
    return this.getWorkItem(workspaceId, workItemId);
  }
  ...
  async cancelWorkItem(workspaceId: string, workItemId: string): Promise<WorkItem> {
    ...
    for (const action of await this.listActions(workspaceId)) {
      if (action.workItemIds.includes(workItemId) && actionIsOpen(action) && action.workItemIds.length === 1) await this.putAction(workspaceId, { ...action, status: "cancelled" });
    }
  ...
  async updateWorkItem(
    ...
    const updated = await this.mutateWorkItem(workspaceId, workItemId, (item) => {
      ...
      return {
        ...
        run: { ...item.run, ...(worktreePath ? { worktreePath, branch } : {}), resumeMessage: [item.run.resumeMessage, "工单已调整：" + note].filter(Boolean).join("\n") }
  ...
  async createDecision(workspaceId: string, input: Omit<DecisionCard, "decisionId" | "createdAt" | "answer">): Promise<DecisionCard> {
    ...
    if (!action && input.workItemId) {
      ...
      action = await this.createAction(workspaceId, { kind: "execute", role: "worker", ownerKey: "worker:" + item.workItemId, workItemIds: [item.workItemId], sessionId: input.sessionId ?? item.run.sessionId, status: "running", stage: "execute", message: "理解用户决定并继续原工单。" });
    }
  ...
  private async deliverDecision(workspaceId: string, card: DecisionCard, message: string, recoveryChoice?: string): Promise<void> {
    ...
    if (card.workItemId) await this.mutateWorkItem(workspaceId, card.workItemId, (item) => ({ ...item,
      ...
      run: action?.kind === "execute" ? { ...item.run, resumeMessage: message, ...(recoveryChoice === "retry" ? { attempts: 0, retryAt: undefined } : {}) } : item.run }));
    ...
    await this.putAction(workspaceId, { ...action, status: waiting ? "decision" : "pending", stage: action.kind === "execute" && action.sessionId ? "deliver" : action.stage,
      message: action.message + "\n" + message, ...(recoveryChoice === "retry" ? { attempts: 0, idleTurns: 0, failure: undefined, retryAt: undefined } : {}),
      history: [...action.history, { at: this.now(), event: "decision.answered", message, decisionId: card.decisionId }] });
  }
