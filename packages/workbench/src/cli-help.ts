...
const states: Partial<Record<WorkbenchRpcMethod, string>> = {
  ...
  "work.confirm": "消息受理状态不明的准备工作；查询实际消息/轮次，确认后才允许继续。",
  ...
  "workItem.continueFrom": "已暂停或人工选择的工单；提供新分支 sessionId 与历史 turnId，迁移当前执行归属并从新分支人工继续。",
  "workItem.confirm": "消息受理状态不明的工单；查询实际消息/轮次，确认后才允许继续。",
...
export function methodHelp(method: string): string | undefined {
  ...
  const desktopHelp: Record<string, { params: string; state: string; example: object }> = {
    ...
    "sessionBrowser.open": { params: "sessionId: string; forceProviderHydration?: boolean; includeWindow?: boolean", state: "桌面在线；进入会话并核对当前历史。includeWindow=false 仅打开，不返回正文窗口；正文可通过 chatTree.get 按版本获取。", example: { sessionId: "<sessionId>" } },
    ...
    "chatTree.get": { params: "sessionId: string; scope?: \"tree\" | \"path\"; knownWindows?: Record<string, { revision: string; cursor?: string }>", state: "桌面在线；默认读取完整树结构。scope=path 返回当前路径；knownWindows 声明已完整持有的成员历史版本与已应用事件水位，服务端省略已覆盖正文，省略不表示删除。未传 knownWindows 时返回路径成员全部正文。", example: { sessionId: "<sessionId>" } },
