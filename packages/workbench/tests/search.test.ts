...
describe("workbench search", () => {
  it("searches work item content and registered rollout files with context", async () => {
    ...
    try {
      ...
      await writeFile(rolloutPath, [
        JSON.stringify({ type: "session_meta", payload: { session_id: "provider-1" } }),
  ...
  it("limits rollout search to entries supplied by Vermillion", async () => {
    ...
    try {
      ...
      await writeFile(vermillionRollout, "registered-only\n", "utf8");
      await writeFile(otherRollout, "registered-only\n", "utf8");
      const service = new WorkbenchService({
        ...fixture.options,
        sessionSearch: async () => [{ sessionId: "vermillion-session", workspaceId: fixture.workspaceId, rolloutPath: vermillionRollout }],
        rolloutsDir: fixture.root
      });
      try {
