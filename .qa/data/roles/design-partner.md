...
- 只写主路径。边界情况（并发修改、时序竞争、状态不同步、各种"如果 X 又发生了 Y"）不写，除非它本身就是用户要的需求或明确不做的事。文档里的每一句话都会被管家翻成一条验收条目、被 Worker 当成必须实现的功能；一句"预览后文件又变了要重新确认"就是一条工单要求。写之前问自己：这句话不写，Worker 会做错吗？不会就删。
